import cv2
from the_utils import (read_video,
                       save_video,
                       measure_distance,
                       draw_player_stats,
...
def run_pipeline(input_video_path, output_video_path='output_videos/output_video.avi'):
    ...
    court_line_path = 'models/model_keypoints.pth'
    Court_line_detector = CourtLineDetector(court_line_path)
    court_line_keypoints = Court_line_detector.predict(video_frames[0])

    # Choose players
    player_detections = player_tracker.choose_and_filter_players(court_line_keypoints, player_detections)
    ...
    output_video_frames = Court_line_detector.draw_keypoints_on_video(output_video_frames, court_line_keypoints)

    # Draw mini court and trajectories
    ...
    for i, frame in enumerate(output_video_frames):
        cv2.putText(frame, f"Frame {i}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)

    # Save the final video
    os.makedirs(os.path.dirname(output_video_path), exist_ok=True)
    save_video(output_video_frames, output_video_path, original_fps)
...
if __name__ == '__main__':
    # You can test it manually with a video path here
    run_pipeline('Input_videos/test.MP4')